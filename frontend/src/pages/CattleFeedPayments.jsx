            ...
            <main className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print" data-tour="header">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900 leading-tight">{t('cattleFeedPayments.pageTitle')}</h1>
                            <p className="text-xs text-gray-400 mt-0.5">{t('cattleFeedPayments.pageSubtitle')}</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3" data-tour="stats">
                    <StatCard label={t('cattleFeedPayments.totalSuppliers')} value={activeSuppliers.length} icon={<Users size={14} />} color="text-blue-600 bg-blue-50 border-blue-100" />
                    <StatCard label={t('cattleFeedPayments.totalAmount')} value={fmt(totalAmount)} icon={<Banknote size={14} />} color="text-emerald-600 bg-emerald-50 border-emerald-100" />
                    <StatCard label={t('cattleFeedPayments.paidCount')} value={`${paidCount} / ${activeSuppliers.length}`} sub={t('cattleFeedPayments.paid')} icon={<CheckCircle2 size={14} />} color="text-violet-600 bg-violet-50 border-violet-100" />
                </div>
                ...
                <div className="flex items-center gap-2 no-print">
                    <div className="relative flex-1 max-w-xs">
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('cattleFeedPayments.searchPlaceholder')} className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-black transition placeholder:text-gray-300" />
                    </div>
                    <div className="flex rounded-xl border border-gray-200 overflow-hidden text-xs font-semibold">
                ...
                <div className="flex flex-col gap-3" data-tour="supplier-list">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-gray-200 gap-2 text-gray-300">
                            <Wheat size={32} />
                            <p className="text-sm">{t('cattleFeedPayments.noSuppliersFound')}</p>
                        </div>
                            ...
                            <div key={supplier.supplier_name} className={`bg-white rounded-2xl border transition ${supplier.is_paid ? "border-emerald-200" : "border-gray-200"}`}>
                                    ...
                                    {supplier.is_paid ? (
                                        <>
                                            <button onClick={(e) => { e.stopPropagation(); downloadReceiptPDF(supplier); }} className="shrink-0 no-print flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-white text-xs font-semibold transition shadow-sm">
                                                <Printer size={11} /> {t('cattleFeedPayments.pdf')}
                                            </button>
                                            <button onClick={(e) => handleUndo(e, supplier)} className="shrink-0 no-print flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-semibold transition disabled:opacity-50 shadow-sm shadow-rose-200">
                                                <RefreshCw size={11} /> {t('cattleFeedPayments.undo')}
                                            </button>
                                        </>
                                    ) : (
                                ...
                                {isOpen && (
                                    <div className="border-t border-gray-100 px-5 py-4 flex flex-col gap-4">
                                        {entries.length > 0 && (
                                            <div>
                                                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">{t('cattleFeedPayments.purchaseDetails')}</p>
                                                <div className="rounded-xl border border-gray-100 overflow-hidden">
                                                    <div className="grid bg-gray-50 border-b border-gray-100" style={{ gridTemplateColumns: "1fr 80px 90px 90px 100px" }}>
                                                        <div className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wide">{t('cattleFeedPayments.feed')}</div>
                                                        <div className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wide text-right">{t('cattleFeedPayments.qty')}</div>
                                                        <div className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wide text-right">{t('cattleFeedPayments.rate')}</div>
                                                        <div className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wide text-right">{t('cattleFeedPayments.mrp')}</div>
                                                        <div className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wide text-right">{t('cattleFeedPayments.amount')}</div>
                                                    </div>
                                                    {entries.map((e, i) => (
                                                        <div key={i} className="grid border-b border-gray-50 last:border-0 hover:bg-gray-50 transition" style={{ gridTemplateColumns: "1fr 80px 90px 90px 100px" }}>
                                                            <div className="px-3 py-2 text-xs text-gray-700">{e.feed_name}</div>
                                                    ...
                                                    <div className="grid bg-gray-50 border-t border-gray-100 font-bold" style={{ gridTemplateColumns: "1fr 80px 90px 90px 100px" }}>
                                                        <div className="px-3 py-2 text-xs text-gray-600 col-span-4 text-right">{t('cattleFeedPayments.total')}</div>
                ...
                {filtered.length > 0 && (
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 border-t border-gray-100 bg-gray-50/60 rounded-b-2xl">
                        <div className="flex items-center gap-2">
                            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-40 transition">{t('cattleFeedPayments.prev')}</button>
                            ...
                            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || totalPages === 0} className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-40 transition">{t('cattleFeedPayments.next')}</button>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-5xl h-[90vh] flex flex-col">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                            <div className="flex items-center gap-3">
                                ...
                                <div><h2 className="text-sm font-bold text-gray-900">{t('cattleFeedPayments.billRegistry')}</h2><p className="text-[10px] text-gray-400">{t('cattleFeedPayments.billRegistryDesc')}</p></div>
                            </div>
                        ...
                        <div className="px-6 py-3 border-b border-gray-100 flex items-center gap-3 flex-wrap shrink-0 bg-gray-50/60">
                            <div className="relative flex-1 min-w-[200px]">
                                <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" />
                                <input autoFocus value={billQuery} onChange={(e) => { setBillQuery(e.target.value); searchBills(e.target.value); setBillDetail(null); }} placeholder={t('cattleFeedPayments.billSearchPlaceholder')} className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-violet-300 transition placeholder:text-gray-300" />
                            ...
                            <button onClick={() => { setBillQuery(""); searchBills(""); setBillDetail(null); }} className="text-xs text-gray-400 hover:text-gray-600 px-3 py-2 rounded-xl border border-gray-200 bg-white transition">{t('cattleFeedPayments.showAll')}</button>
            ...
            {deleteConfirmOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-md">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                            <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-rose-100 flex items-center justify-center"><Trash2 size={18} className="text-rose-600" /></div><div><h2 className="text-sm font-bold text-gray-900">{t('cattleFeedPayments.deleteBill')}</h2><p className="text-[10px] text-gray-400">{t('cattleFeedPayments.deleteWarning')}</p></div></div>
                        ...
                        <div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-100">
                            <button onClick={cancelDeleteBill} className="px-4 py-2 rounded-xl text-xs font-semibold border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition">{t('cattleFeedPayments.cancel')}</button>
