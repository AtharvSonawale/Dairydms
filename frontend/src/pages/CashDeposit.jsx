...
function Field({ label, icon, children }) {
    return (
        <div className="flex flex-col gap-1.5 shrink-0 self-end">
            <span className="flex items-center gap-1 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                {icon}{label}
            </span>
            {children}
        </div>
...
function TypeToggle({ value, onChange, t }) {
    return (
        <div className="flex rounded-xl border border-gray-200/60 overflow-hidden text-xs font-semibold shadow-sm">
            {[
                { val: "credit", label: t('cashDeposit.creditAdd'), active: "bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg shadow-emerald-500/30" },
                { val: "debit", label: t('cashDeposit.debitWithdraw'), active: "bg-gradient-to-br from-rose-500 to-rose-600 text-white shadow-lg shadow-rose-500/30" },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100/50">
            <main className="max-w-screen mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-200/50 p-5">
                    <div>
                        <div className="flex items-center gap-2.5 text-sm text-gray-600 mb-1">
                            ...
                            <span>{t('cashDeposit.pageBreadcrumb', { defaultValue: 'Finance' })}</span>
                    ...
                    <div className="flex items-center gap-2 flex-wrap" data-tour="deposit-header-actions">
                        ...
                        <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{t('cashDeposit.dateLabel')}</span>
                        ...
                        <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{t('cashDeposit.downloadPDF')}</span>
                ...
                {/* ── Stats ── */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4" data-tour="deposit-stats">
                    {[
                        { label: rangeMode === "daily" ? t('cashDeposit.entriesToday') : t('cashDeposit.entriesInRange'), value: activeData.length, icon: <PiggyBank size={16} />, color: "from-blue-50 to-blue-100/50 border-blue-200/60 text-blue-700" },
                        { label: t('cashDeposit.totalCredited'), value: `₹${fmt(totalCredit)}`, icon: <TrendingUp size={16} />, color: "from-emerald-50 to-emerald-100/50 border-emerald-200/60 text-emerald-700" },
                        { label: t('cashDeposit.totalDebited'), value: `₹${fmt(totalDebit)}`, icon: <TrendingDown size={16} />, color: "from-rose-50 to-rose-100/50 border-rose-200/60 text-rose-600" },
                        {
                ...
                {/* ── Entry Form ── */}
                {can('cash_advance', 'C') && (
                    <div className="relative z-20 rounded-2xl border border-gray-200/60 bg-white/80 backdrop-blur-sm shadow-lg shadow-gray-200/50 px-6 py-5" data-tour="deposit-form">
                            ...
                            <Field label={t('cashDeposit.seller')} icon={<User size={12} />}>
                                <div className="relative" style={{ width: "160px" }} data-seller-anchor>
                                    <TinyInput
                                        ...
                                        style={{ width: "160px" }}
                                    />
                                    {dropdownOpen && !form.seller_id && filteredSellers.length > 0 && createPortal(
                                        <div
                                            style={{
                                                position: "fixed",
                                                top: (document.querySelector('[data-seller-anchor]')?.getBoundingClientRect().bottom ?? 0) + 4,
                                                left: document.querySelector('[data-seller-anchor]')?.getBoundingClientRect().left ?? 0,
                                            }}
                                        ...
                                        {[{ val: "refund", label: "Refund" }, { val: "withdrawal", label: "Withdrawal" }].map(({ val, label }) => (
                                            <button key={val} type="button" onClick={() => set("category", val)}
                                                className={`px-3 py-[7px] transition-colors
                                                    ${form.category === val ? "bg-gradient-to-br from-rose-500 to-rose-600 text-white shadow-lg shadow-rose-500/30" : "bg-white/60 backdrop-blur-sm text-gray-500 hover:bg-gray-50/80"}`}>
                                                {label}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        {selectedSeller && (
                            <div className="mt-4 pt-4 border-t border-gray-200/60 relative z-10">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {sellerDepositRate && (
                                            <span className="text-[10px] px-2.5 py-1 rounded-lg bg-blue-50/80 backdrop-blur-sm border border-blue-200/60 text-blue-600 font-semibold shadow-sm">
                                                {t('cashDeposit.rate')}: ₹{sellerDepositRate}/{t('cashDeposit.litre')}
                                            </span>
                                        )}
                                        {milkSummary && sellerDepositRate && (
                                            <button
                                                onClick={handleAutoFill}
                                                className="text-[10px] px-2.5 py-1 rounded-lg bg-violet-50/80 backdrop-blur-sm border border-violet-200/60 text-violet-600 font-semibold hover:bg-violet-100/80 transition shadow-sm"
                                            >
                                                {t('cashDeposit.autoFill')} ₹{(milkSummary.qty * sellerDepositRate).toFixed(2)}
                                            </button>
                                        )}
                                    </div>
                                </div>
                                ...
                                {loadingBal ? (
                                    ...
                                    <div className="flex gap-3 flex-wrap mt-2">
                                        {[
                                            { label: t('cashDeposit.totalCredited'), value: balance.total_credit, color: "from-emerald-50 to-emerald-100/50 border-emerald-200/60 text-emerald-700" },
                                            { label: t('cashDeposit.totalDebited'), value: balance.total_debit, color: "from-rose-50 to-rose-100/50 border-rose-200/60 text-rose-600" },
                                            {
                                            ...
                                            <div key={label} className={`relative overflow-hidden rounded-xl border bg-gradient-to-br ${color} shadow-sm px-4 py-2.5 flex flex-col gap-0.5`}>
                                                <div className="absolute -right-6 -top-6 w-20 h-20 rounded-full bg-white/20 blur-2xl" />
                                                <p className="text-[10px] font-semibold uppercase tracking-wider opacity-60 relative z-10">{label}</p>
                                                <p className="text-base font-bold leading-tight relative z-10">₹{fmt(value)}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {!sellerDepositRate && (
                                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-50/80 backdrop-blur-sm border border-amber-200/60 text-amber-700 text-xs shadow-sm">
                            ...
                            <button type="button" onClick={editingEntry ? handleUpdate : handleSave}
                                ...
                                ${(editingEntry ? editSaving : saving) || !form.seller_id
                                        ...
                                        : editingEntry
                                            ...
                                            : form.type === "credit"
                                                ? "bg-gradient-to-br from-emerald-500 to-emerald-600 shadow-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/40"
                ...
                {/* ── Entries Table ── */}
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-200/50 overflow-hidden" data-tour="deposit-table">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200/60 bg-gradient-to-r from-gray-50/50 to-white/50">
                        ...
                        {form.seller_id && selectedSeller && (
                            <span className="text-xs font-semibold text-blue-600 bg-blue-50/80 backdrop-blur-sm border border-blue-200/60 px-2 py-1 rounded-lg shadow-sm">
                    ...
                    <div className="grid border-b border-gray-200/60 bg-gradient-to-r from-gray-50/50 to-white/50" style={{ gridTemplateColumns: GRID }}>
                        {COLS.map(label => (
                            <div key={label} className="px-3 py-3 flex items-center text-[11px] font-semibold text-gray-500 uppercase tracking-wide border-r border-gray-200/60 last:border-r-0">
                                {label}
                            </div>
                        ))}
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-300">
                            <PiggyBank size={40} className="text-gray-200" />
                            ...
                            <div className="min-w-max">
                                {paginatedEntries.map((r, i) => (
                                    <div key={r.id || i}
                                        ...
                                        {form.seller_id && (
                                            <TableCell>
                                                <span className={`font-mono font-semibold text-xs px-2.5 py-1 rounded-lg backdrop-blur-sm shadow-sm
                                        ...
                                        {isAdmin && (
                                            <TableCell>
                                                <button onClick={() => handleEdit(r)}
                                                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition border backdrop-blur-sm shadow-sm
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                                        ...
                                        ${currentPage === p ? 'bg-gradient-to-br from-gray-900 to-gray-800 text-white border-gray-900 shadow-lg shadow-gray-900/30' : 'bg-white/60 backdrop-blur-sm text-gray-500 border-gray-200/60 hover:border-gray-300 hover:bg-gray-50/80'}`}>
                                                    {p}
                                                </button>
                                        )}
                                </div>
                    ...
                    {/* ── Totals Footer ── */}
                    {activeData.length > 0 && (
                        <div className="bg-white/80 backdrop-blur-sm rounded-b-2xl border-t-2 border-gray-200/60 overflow-x-auto">
                            <div className="grid bg-gradient-to-r from-gray-50/50 to-white/50 min-w-max" style={{ gridTemplateColumns: GRID }}>
                                ...
                                <div className="px-3 py-2.5 border-r border-gray-200/60" />
                                <div className="px-3 py-2.5 border-r border-gray-200/60" />
                                <div className="px-3 py-2.5 text-xs font-bold text-blue-700 border-r border-gray-200/60">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-gray-400 pb-2 pt-2 border-t border-gray-200/40">
                        ...
                        <span>· {t('cashDeposit.footerRole', { defaultValue: 'Role' })}: <strong className="text-gray-600">{t('status.admin')}</strong></span>
                    </div>
                </div>
            </main>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-200/60 p-6 w-[340px] flex flex-col gap-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h2 className="text-gray-800 font-bold text-base">Delete Entry?</h2>
