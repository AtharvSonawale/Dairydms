// FeedFulfillmentScanner.jsx
import { useEffect, useRef, useState, useCallback } from "react";
import jsQR from "jsqr";
import api from "../api/axios";
import {
    CheckCircle2, XCircle, Camera, Loader2, PackageCheck,
    Home, Droplets, AlertTriangle, ArrowLeft
} from "lucide-react";
import { Link } from "react-router-dom";

export default function FeedFulfillmentScanner() {
    ...
    const [preview, setPreview] = useState(null); // { transaction_id, status, items }
    ...
    const fetchPreview = async (token) => {
        ...
        setResult(null);
        try {
            const { data } = await api.get(`/cattle-feed-sales/fulfillment/${token}`);
    ...
    const startCamera = async () => {
        ...
        try {
            let stream;
            try {
                // Prefer rear camera on phones; on laptops this constraint is
                // just "ideal" so it should fall back — but some browsers
                // throw OverconstrainedError instead, so we catch and retry.
    ...
    const handleConfirm = async () => {
        ...
        setConfirming(true);
        try {
            const { data } = await api.post(`/cattle-feed-sales/fulfillment/${preview.token}/confirm`);
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-200/50 px-5 py-4">
                    <div>
                        <div className="flex items-center gap-2.5 text-sm text-gray-600 mb-0.5">
                            ...
                            <span className="font-medium">Feed Pickup Scanner</span>
                ...
                {!scanning && !preview && !result && (
                    <>
                        ...
                        <div className="flex flex-col gap-3 pt-2 border-t border-gray-200/60">
                            <label className="text-[10.5px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                                <Droplets size={12} /> Or enter token manually
                            </label>
                            ...
                            <div className="flex flex-col gap-1 pl-7">
                                {result.items.map((item, i) => (
                                    <span key={i} className="text-xs text-gray-600">
                                        {item.feed_name}: {parseFloat(item.quantity).toFixed(2)} {item.unit}
                                    </span>
                                ))}
                            </div>
                        )}
