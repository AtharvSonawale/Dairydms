...
// seller_type is now FIXED per page (passed in via props), not user-editable
...
// ── Base Page ─────────────────────────────────────────────────
// sellerType: "Utpadak" | "Gavali" — fixed for the lifetime of this page.
export default function MilkEntryBase({ sellerType }) {
    ...
    // RS232 Machine Quantity — driven by the backend's live weight machine reader.
    // We still track all three subtype slots because the "Default Scale" (weight)
    // is used as a fallback whenever per-type auto-routing is disabled in Port Settings.
    ...
    // Read-only: whether auto-routing (by seller type) is enabled. Configured
    // globally from Port Settings — no toggle lives on this page anymore since
    // each page now only ever deals with one seller type.
    ...
    // The scale that feeds this form: the seller-type-specific scale when
    // auto-routing is on, otherwise the single Default Scale.
    ...
    // ── Load the weight/fat machines' configured ports ──
    useEffect(() => {
    ...
    // ── Load the (global) port-switching flag — read only on this page ──
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const handleWeightUpdate = (subtypeKey) => (reading) => {
            ...
            // Ltr slot only (reading.value2) unless the configured default
            // unit for this scale is Kg — the backend (weightMachine.service.js)
            // has already derived Ltr via the 0.97 factor when needed.
            ...
            if (fillValue !== null && fillValue !== undefined) {
                setForm(p => {
                    const isActive = portSwitchingEnabledRef.current
                        ...
                        : subtypeKey === "weight"; // auto-routing off → only the Default Scale drives the form
    ...
    // ── Auto-connect the moment this page mounts. Only connects the scale
    // relevant to this seller type (plus the Default Scale as a fallback) —
    // no need to open the "other" type's serial port on this page. ──
            ...
            steps: [
                {
                    ...
                    popover: { title: 'Select Seller', description: `Search and pick the ${sellerType} seller for this entry.` },
                },
                {
                ...
                },
                {
                    ...
                    popover: { title: 'Entries Table', description: `All saved ${sellerType} entries for the selected date appear here.` },
                },
            ],
        });
        driverObj.drive();
    ...
    // Only sellers registered as this page's seller type show up here
    ...
    // Entries are filtered server-side via ?seller_type=Utpadak|Gavali
    ...
    const handleEdit = (entry) => {
        ...
        if (entry.machine_qty) {
            const key = sellerType === "Gavali" ? "weight_gavali" : "weight_utpadak";
            ...
            <main className="max-w-screen mx-auto px-4 sm:px-6 py-8 flex flex-col gap-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl ${theme.headerIcon} flex items-center justify-center shadow-md shadow-gray-200`}>
                            <Droplets size={18} className="text-white" />
                        </div>
                        <div>
                            <h1 className="text-xl font-bold text-gray-900 leading-tight">
                                {sellerType} {t('milkEntry.pageTitle')}
                            </h1>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-5">
                    <div className="flex items-center justify-between mb-4">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
                            {editingEntry ? t('milkEntry.editEntry') : t('milkEntry.newEntry')} · {sellerType}
                        </p>
                        {editingEntry && (
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <span className="block text-[10px] font-bold text-emerald-300 uppercase tracking-widest">
                                        Weight · {activeWeightKey === "weight" ? "Default Scale" : `${sellerType} Scale`}
                                    </span>

                                    {!portSwitchingEnabled && (
                                        <span className="block text-[9px] text-gray-400 mt-0.5">
                                            Auto-routing off — using Default Scale (configure in Port Settings)
                                        </span>
                                    )}

                                    <span
                                        className={`flex items-center gap-1.5 text-xs font-mono mt-0.5 ${isMachineConnected ? "text-emerald-300" : "text-gray-400"
                                            }`}
                                    >
                                        <span
                    ...
                    <div className="flex items-start gap-3 flex-wrap" onKeyDown={handleFormKeyDown}>
                        ...
                        {/* Seller type is fixed for this page — shown as a static badge */}
                        <Field label={t('milkEntry.sellerTypeLabel')} icon={<User size={12} />}>
                            <div className={`h-[35px] px-3 flex items-center rounded-xl border text-xs font-bold whitespace-nowrap ${theme.badge}`}>
                                {sellerType}
                            </div>
                        </Field>
                ...
                {entries.length > 0 && (
                    <div className="bg-white rounded-b-2xl border-t-2 border-gray-100 overflow-x-auto">
                        <div className="grid bg-gray-50/80 min-w-max"
                            ...
                            <div className="px-3 py-2.5 border-r border-gray-100" />
                            <div className={isAdmin ? "px-3 py-2.5 border-r border-gray-100" : "px-3 py-2.5"} />
