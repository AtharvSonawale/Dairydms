    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100/50">
            <main className="max-w-screen mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-200/50 px-5 py-4">
                    <div>
                        <div className="flex items-center gap-2.5 text-sm text-gray-600 mb-1">
                            ...
                            <span>{sellerType} {t('milkEntry.pageTitle')}</span>
                            <span className={`flex items-center gap-1.5 px-3 py-1 rounded-xl ${theme.headerIcon} text-white text-xs font-semibold shadow-md shadow-gray-900/30`}>
                                <Droplets size={12} /> {sellerType}
                            </span>
                        </div>
                ...
                <CompactCard>
                    <SectionHeader
                        icon={editingEntry ? <Pencil size={16} className="text-white" /> : <Droplets size={16} className="text-white" />}
                        title={editingEntry ? t('milkEntry.editEntry') : t('milkEntry.newEntry')}
                        sub={editingEntry ? `${sellerType} · ${new Date(editingEntry.entry_date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : sellerType}
                        action={editingEntry && (
                ...
                <CompactCard>
                    <SectionHeader
                        ...
                        title={t('milkEntry.entries')}
                        sub={`${sellerType} · ${new Date(selectedDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}`}
                    />

                    <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200/60 bg-gradient-to-r from-gray-50/50 to-white/50 -mx-5 -mt-1 mb-3 rounded-t-2xl">
                        <input
                            type="text"
                            value={searchName}
