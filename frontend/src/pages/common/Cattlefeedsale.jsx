...
function SpeedFeedConfigModal({ open, onClose, feeds, showFlash }) {
    ...
    const fetchSpeed = async () => {
        ...
        try {
            ...
            setSpeedFeeds(data);
        } catch { showFlash('error', 'Failed to load speed feeds'); }
    ...
    const handleSave = async () => {
        if (!form.feed_id && !editingId) {
            showFlash('error', 'Select a feed'); return;
        }
        setSaving(true);
        try {
            const payload = {
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-bold text-gray-900">Speed Feeds Config</h2>
                            <p className="text-[10px] text-gray-400">Add quick-tap feeds with images</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-1 min-h-0 overflow-hidden">
                    ...
                    <div className="w-64 shrink-0 border-r border-gray-100 px-5 py-4 flex flex-col gap-4 overflow-y-auto">
                        ...
                        {/* Feed select (only when adding) */}
                        {!editingId && (
                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Feed</label>
                                <select
                                    value={form.feed_id}
                        ...
                        {/* Image upload */}
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Feed Image</label>
                    ...
                    {/* Right — Current Speed Feeds */}
                    <div className="flex-1 overflow-y-auto px-5 py-4">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">
                            {speedFeeds.length} Speed Feed{speedFeeds.length !== 1 ? 's' : ''}
                        </p>
                        ...
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-12 gap-2 text-gray-300">
                                <Zap size={28} />
                                <p className="text-xs">No speed feeds yet</p>
                            </div>
                        ) : (
        ...
        <div className="pb-4 mb-4 border-b border-gray-100">
            ...
            <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                    <Zap size={10} className="text-amber-500" /> Quick ({speedFeeds.length})
                </span>
...
export default function CattleFeedSales() {
            ...
            steps: [
                {
                    ...
                    popover: { title: 'Date & PDF', description: 'Pick a date or range, switch between daily/weekly/monthly, and download a PDF report. Use Speed Config to set up quick‑tap feed buttons.' },
                },
                {
                    element: '[data-tour="sales-stats"]',
                    popover: { title: 'Today\'s Summary', description: 'Live counts of sales, total revenue, and unique sellers served.' },
                },
                {
                    element: '[data-tour="sales-form"]',
                    popover: { title: 'New Sale Entry', description: 'Search for a seller, add feed lines via the quick strip or search, and record the sale. Totals update automatically.' },
                },
                {
                    element: '[data-tour="sales-table"]',
                    popover: { title: 'Transactions', description: 'List of all sales in the selected period. Print receipts, edit quantities/rates, or delete a sale (stock is reversed).' },
                },
            ],
        });
        driverObj.drive();
    ...
    const fetchRangeEntries = async (from = fromDate, to = toDate) => {
        ...
        try {
            ...
            showFlash("error", "Failed to load range entries");
        } finally {
    ...
    const handleEditSave = async () => {
        ...
        try {
            ...
            setEditingSale(null);
            showFlash("success", "Sale updated");
        } catch (err) {
    ...
    const fetchSales = async (date) => {
        ...
        try {
            ...
            showFlash("error", "Failed to load sales");
        } finally {
            setLoading(false);
        }
    };
    ...
    // ── FIX: use existing product_sale_enabled column ──
    const filteredSellers = (() => {
        const sorted = [...sellers]
            .filter((s) => s.product_sale_enabled == 1)   // changed from cattle_feed_sale_enabled
    ...
    const handleSellerSearchChange = (val) => {
        ...
        const exact = sellers.find(
            (s) =>
                s.product_sale_enabled == 1 &&   // changed from cattle_feed_sale_enabled
    ...
    const handleSave = async () => {
        if (!form.seller_id) { showFlash("error", "Please select a seller"); return; }
        ...
        for (const l of validLines) {
            ...
            if (feed && parseFloat(l.quantity) > parseFloat(feed.current_stock || 0)) {
                showFlash("error", `Insufficient stock for ${feed.feed_name}. Available: ${parseFloat(feed.current_stock).toFixed(2)} ${feed.unit}`);
                return;
            }
        }
        ...
        try {
            ...
            await fetchSales(selectedDate);
            await fetchFeeds();
            showFlash("success", "Sale recorded");
            setForm(EMPTY_FORM);
    ...
    const handleDownloadPDF = () => {
        ...
        const modeLabel = rangeMode === "daily" ? "Daily"
            : rangeMode === "weekly" ? "Weekly"
                : rangeMode === "monthly" ? "Monthly" : "Custom";
        const periodLabel = fromDate === toDate ? fmtD(fromDate) : `${fmtD(fromDate)} to ${fmtD(toDate)}`;
        ...
        <title>Cattle Feed Sales — ${periodLabel}</title>
        <style>
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px;border-bottom:2px solid #000;padding-bottom:10px">
        <div>
            <div style="font-size:18px;font-weight:bold;color:#000">Cattle Feed Sales</div>
            <div style="font-size:11px;color:#333;margin-top:3px">${modeLabel} Report · ${periodLabel}</div>
            <div style="font-size:10px;color:#555;margin-top:2px">Generated ${new Date().toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true })}</div>
        </div>
        <div style="display:flex;gap:10px">
            <div style="background:#f2f2f2;border:1px solid #999;padding:8px 14px;border-radius:4px;text-align:center">
                <div style="font-size:9px;color:#333;font-weight:600;text-transform:uppercase">Sales</div>
                <div style="font-size:16px;font-weight:700;color:#000">${baseData.length}</div>
            </div>
            <div style="background:#f2f2f2;border:1px solid #999;padding:8px 14px;border-radius:4px;text-align:center">
                <div style="font-size:9px;color:#333;font-weight:600;text-transform:uppercase">Sellers</div>
                <div style="font-size:16px;font-weight:700;color:#000">${uniqueSellersCount}</div>
            </div>
            <div style="background:#f2f2f2;border:1px solid #999;padding:8px 14px;border-radius:4px;text-align:center">
                <div style="font-size:9px;color:#333;font-weight:600;text-transform:uppercase">Revenue</div>
    ...
    <table>
        <thead>
            <tr style="background:#000;color:#fff">
                <th style="padding:5px 6px;border:1px solid #444;font-size:9px;text-align:left;width:22%">Seller</th>
                <th style="padding:5px 6px;border:1px solid #444;font-size:9px;text-align:left;width:20%">Feed</th>
                <th style="padding:5px 6px;border:1px solid #444;font-size:9px;text-align:right;width:12%">Qty</th>
                <th style="padding:5px 6px;border:1px solid #444;font-size:9px;text-align:right;width:12%">Rate</th>
                <th style="padding:5px 6px;border:1px solid #333;background:#333;font-size:9px;text-align:right;width:14%">Amount</th>
                <th style="padding:5px 6px;border:1px solid #444;font-size:9px;text-align:left;width:20%">Date / Time</th>
            </tr>
        </thead>
        <tbody>
            ${rows}
            <tr style="background:#e0e0e0;font-weight:bold;border-top:2px solid #000">
                <td colspan="2" style="padding:5px 6px;border:1px solid #999;font-size:9px;font-weight:700;color:#000">Grand Total — ${baseData.length} entries · ${uniqueSellersCount} seller${uniqueSellersCount !== 1 ? "s" : ""}</td>
    ...
    `).join("");
        ...
        <div class="title">Cattle Feed Cash Memo</div>
    ...
    // table
    const COLS = ['Seller', 'Feed', 'Qty', 'Rate', 'Total', 'Time', ''];
            ...
            <main className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900 leading-tight">Cattle Feed Sales</h1>
                            <p className="text-xs text-gray-400 mt-0.5">
                                Record and manage feed sales —{" "}
                ...
                focus:outline-none focus:ring-2 focus:ring-black transition" />
                        ...
                        <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Download PDF</span>
                            <div className="flex flex-wrap items-center gap-1.5">
                                <div className="flex rounded-xl border border-gray-200 overflow-hidden text-xs font-semibold">
                                    {[{ v: "daily", l: "Day" }, { v: "weekly", l: "Week" }, { v: "monthly", l: "Month" }, { v: "custom", l: "Custom" }].map(({ v, l }) => (
                ...
                <div className="grid grid-cols-3 sm:grid-cols-3 gap-3" data-tour="sales-stats">
                    {[
                        { label: "Today's Sales", value: sales.length, icon: <ShoppingCart size={14} />, color: "text-blue-600 bg-blue-50 border-blue-100" },
                        { label: "Total Revenue", value: "₹" + totalRevenue.toFixed(2), icon: <TrendingUp size={14} />, color: "text-emerald-600 bg-emerald-50 border-emerald-100" },
                        { label: "Sellers Served", value: uniqueSellers, icon: <Users size={14} />, color: "text-violet-600 bg-violet-50 border-violet-100" },
                ...
                {can('cattle_feed_sales', 'C') && (
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-5" data-tour="sales-form" onKeyDown={handleFormKeyDown}>
                        ...
                        <div className="flex items-start gap-3 flex-wrap pb-4 mb-4 border-b border-gray-100">
                            <Field label="Seller" icon={<User size={12} />}>
                                <div className="relative" style={{ width: "220px" }}>
                                    <TinyInput
                                        ...
                                        placeholder="Search seller by name or code"
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {grandFormTotal > 0 && (
                                <div className="flex items-center gap-2 text-sm font-bold text-gray-800">
                                    <span className="text-xs text-gray-400 font-medium">{lines.filter(l => l.feed_id).length} feed{lines.filter(l => l.feed_id).length !== 1 ? "s" : ""} ·</span>
                                    Grand Total:
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                            <p className="text-xs text-gray-400">
                                {sales.length} {sales.length === 1 ? "sale" : "sales"} on{" "}
                                {new Date(selectedDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                                {totalRevenue > 0 && (
                                    <span className="ml-2 text-emerald-600 font-semibold">· ₹{totalRevenue.toFixed(2)} total</span>
                                )}
                            </p>
                    ...
                    {activeData.length > 0 && (
                        <div className="grid border-t-2 border-gray-100 bg-gray-50/80"
                            ...
                            <div className="px-3 py-2.5 text-xs font-bold text-gray-600 border-r border-gray-100">
                                {activeData.length} {activeData.length === 1 ? "entry" : "entries"}
                            </div>
                ...
                <div className="flex flex-wrap gap-4 text-xs text-gray-400">
                    <span>• <strong className="text-gray-600">{activeData.length}</strong> {rangeMode === "daily" ? "sales recorded today" : "sales in range"}</span>
                    <span>• Stock updates automatically when recording or deleting</span>
                    <span>• Click a feed card to add it quickly</span>
                </div>
                        ...
                        <div className="flex flex-col gap-2">
                            {editingSale.items.map((item, index) => (
                                <div key={item.sale_id} className="grid grid-cols-3 gap-2 items-center">
                                    <div className="px-3 py-2 rounded-xl bg-gray-50 border border-gray-100">
                                        <p className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Feed</p>
                                        <p className="text-sm font-semibold text-gray-800 mt-0.5 truncate">
                                            {item.feed_name}
                                        </p>
                                    </div>
