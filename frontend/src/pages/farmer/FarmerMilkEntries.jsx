...
import {
    ...
    Home, Sun, Moon, Droplets, BadgeCheck, X
} from "lucide-react";
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-200/50 px-5 py-4">
                    <div>
                        <div className="flex items-center gap-2.5 text-sm text-gray-600 mb-1">
                            ...
                            <Link to="/farmer/dashboard" className="hover:text-gray-800 transition">
                                {t('dashboard.myDashboard', { defaultValue: 'My Dashboard' })}
                            </Link>
                            <ChevronRight size={12} className="text-gray-300" />
                            <span>{t('dashboard.myMilkEntries', { defaultValue: 'My Milk Entries' })}</span>
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 text-white text-xs font-semibold shadow-md shadow-emerald-500/30">
                                <Milk size={12} /> {t('status.farmer', { defaultValue: 'Farmer' })}
                            </span>
                        </div>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-gray-200/60 bg-white/80 backdrop-blur-sm shadow-lg shadow-gray-200/50 p-5">
                    ...
                    <div className="relative z-10">
                        ...
                        {loading ? (
                            ...
                            <EmptyState icon={<Milk size={32} />} msg={t('dashboard.noMilkEntries', { defaultValue: 'No milk entries for this period' })} />
                        ) : (
                                        ...
                                        <tbody className="divide-y divide-gray-100/60">
                                            {paginated.map((e) => (
                                                <tr key={e.entry_id} className="hover:bg-gray-50/30 transition">
                                                    <td className="px-4 py-2.5 text-xs text-gray-600 font-mono whitespace-nowrap border-r border-gray-200/60">{fmtDate(e.entry_date)}</td>
                                                    ...
                                                    <td className="px-4 py-2.5 font-mono text-blue-600 font-semibold border-r border-gray-200/60">{parseFloat(e.quantity).toFixed(2)}</td>
                                                    <td className="px-4 py-2.5 font-mono text-amber-600 border-r border-gray-200/60">{parseFloat(e.fat).toFixed(2)}</td>
                                                    <td className="px-4 py-2.5 font-mono text-violet-600 border-r border-gray-200/60">{parseFloat(e.snf).toFixed(2)}</td>
                                                    <td className="px-4 py-2.5 font-mono text-gray-500 border-r border-gray-200/60">{parseFloat(e.water || 0).toFixed(2)}</td>
