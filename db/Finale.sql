...
CREATE TABLE bill_master (
   ...
   has_custom_commission tinyint(1) DEFAULT '0',
   custom_commission_ids json DEFAULT NULL,
   PRIMARY KEY (bill_id),
...
CREATE TABLE bill_milk_entries (
   ...
   created_at timestamp NULL DEFAULT CURRENT_TIMESTAMP,
   is_custom_commission tinyint(1) DEFAULT '0',
   custom_commission_id int DEFAULT NULL,
   PRIMARY KEY (id),
   KEY bill_id (bill_id),
   KEY centre_id (centre_id),
   KEY idx_custom_commission (is_custom_commission,custom_commission_id),
...
CREATE TABLE bonus_default_slabs (
   slab_id int NOT NULL AUTO_INCREMENT,
   centre_id int NOT NULL,
   fat_min decimal(4,2) NOT NULL,
   fat_max decimal(4,2) NOT NULL,
   bonus decimal(6,2) DEFAULT '0.00',
   vahatuk decimal(6,2) DEFAULT '1.00',
   rate decimal(6,2) NOT NULL,
   sort_order int DEFAULT '0',
   PRIMARY KEY (slab_id),
   KEY idx_centre_id (centre_id),
   CONSTRAINT bonus_default_slabs_ibfk_1 FOREIGN KEY (centre_id) REFERENCES centres (centre_id) ON DELETE CASCADE
...
CREATE TABLE buffalo_milk_rates (
 ...
 ) ENGINE=InnoDB AUTO_INCREMENT=22716 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE cow_milk_rates (
 ...
 ) ENGINE=InnoDB AUTO_INCREMENT=7683 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE gavali_bonus_default_rates (
   id int NOT NULL AUTO_INCREMENT,
   centre_id int NOT NULL,
   ...
   CONSTRAINT gavali_bonus_default_rates_ibfk_1 FOREIGN KEY (centre_id) REFERENCES centres (centre_id) ON DELETE CASCADE,
   CONSTRAINT gavali_bonus_default_rates_ibfk_2 FOREIGN KEY (updated_by) REFERENCES operators (operator_id)
...
CREATE TABLE milk_entries (
   ...
   original_rate_applied decimal(8,2) DEFAULT NULL,
   is_premium tinyint(1) DEFAULT '0',
   total_amount decimal(10,2) NOT NULL,
   original_total_amount decimal(10,2) DEFAULT NULL,
   entry_time datetime DEFAULT CURRENT_TIMESTAMP,
   rate_corrected_at datetime DEFAULT NULL,
   rate_corrected_by int DEFAULT NULL,
   ...
   KEY fk_milk_entries_corrected_by (rate_corrected_by),
   CONSTRAINT fk_milk_admin FOREIGN KEY (created_by_admin_id) REFERENCES admins (admin_id) ON DELETE CASCADE ON UPDATE CASCADE,
   CONSTRAINT fk_milk_entries_corrected_by FOREIGN KEY (rate_corrected_by) REFERENCES admins (admin_id) ON DELETE SET NULL,
 ...
 ) ENGINE=InnoDB AUTO_INCREMENT=2636 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE port_settings (
   id int NOT NULL AUTO_INCREMENT,
   dairy_id int NOT NULL,
   serial_port varchar(50) NOT NULL DEFAULT '',
   serial_baud_rate varchar(20) NOT NULL DEFAULT '9600',
   serial_data_bits varchar(5) NOT NULL DEFAULT '8',
   serial_stop_bits varchar(5) NOT NULL DEFAULT '1',
   serial_parity varchar(10) NOT NULL DEFAULT 'none',
   kg_unit_label varchar(10) NOT NULL DEFAULT 'Kg',
   ltr_unit_label varchar(10) NOT NULL DEFAULT 'Ltr',
   default_weight_unit enum('kg','ltr') NOT NULL DEFAULT 'ltr',
   machine_type enum('weight_gavali','weight_utpadak','weight','fat') NOT NULL,
   updated_at timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   PRIMARY KEY (id),
   UNIQUE KEY uq_dairy_machine_type (dairy_id,machine_type),
   KEY idx_dairy_id (dairy_id),
   CONSTRAINT port_settings_ibfk_1 FOREIGN KEY (dairy_id) REFERENCES dairies (dairy_id) ON DELETE CASCADE
...
CREATE TABLE rate_adjustments (
   ...
   entry_date date NOT NULL,
   milk_type enum('cow','buffalo','mixed') NOT NULL,
   original_rate decimal(8,2) NOT NULL,
   corrected_rate decimal(8,2) NOT NULL,
   ...
   created_by int DEFAULT NULL,
   created_at datetime DEFAULT CURRENT_TIMESTAMP,
   settled_in_bill_id bigint DEFAULT NULL,
   settled_at datetime DEFAULT NULL,
   PRIMARY KEY (id),
   KEY idx_seller_status (seller_id,status),
   KEY idx_entry_id (entry_id),
   KEY idx_centre_id (centre_id),
   KEY rate_adjustments_ibfk_4 (settled_in_bill_id),
   CONSTRAINT rate_adjustments_ibfk_1 FOREIGN KEY (entry_id) REFERENCES milk_entries (entry_id) ON DELETE CASCADE,
   CONSTRAINT rate_adjustments_ibfk_2 FOREIGN KEY (seller_id) REFERENCES sellers (seller_id),
   CONSTRAINT rate_adjustments_ibfk_3 FOREIGN KEY (centre_id) REFERENCES centres (centre_id),
   CONSTRAINT rate_adjustments_ibfk_4 FOREIGN KEY (settled_in_bill_id) REFERENCES bill_master (bill_id) ON DELETE SET NULL
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE seller_deposits (
   ...
   category enum('refund','withdrawal') DEFAULT NULL,
   amount decimal(10,2) NOT NULL,
   remarks text,
   transaction_date date NOT NULL,
   created_at datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE sellers (
   ...
   seller_type enum('Utpadak','Gavali') NOT NULL DEFAULT 'Utpadak',
   milk_type enum('cow','buffalo','both') NOT NULL DEFAULT 'cow',
   ...
   product_sale_rate decimal(8,2) DEFAULT NULL,
   cattle_feed_sale_enabled tinyint(1) NOT NULL DEFAULT '0',
   cheque longtext COMMENT 'Base64 encoded cheque image',
   profile_image longtext COMMENT 'Base64 encoded profile photo',
...
CREATE TABLE transaction_sequences (
   id int NOT NULL AUTO_INCREMENT,
   centre_id int NOT NULL,
   sale_type varchar(30) NOT NULL DEFAULT 'cattle_feed',
   financial_year varchar(10) NOT NULL,
   last_number int NOT NULL DEFAULT '0',
   PRIMARY KEY (id),
   UNIQUE KEY uq_centre_type_fy (centre_id,sale_type,financial_year),
   CONSTRAINT transaction_sequences_ibfk_1 FOREIGN KEY (centre_id) REFERENCES centres (centre_id) ON DELETE CASCADE
 ) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE user_favourites (
   id int NOT NULL AUTO_INCREMENT,
   user_role enum('admin','operator','seller') NOT NULL,
   ...
   created_at datetime DEFAULT CURRENT_TIMESTAMP,
   PRIMARY KEY (id),
   UNIQUE KEY uq_user_navpath (user_role,user_id,nav_path),
   KEY idx_user (user_role,user_id),
   KEY centre_id (centre_id),
   CONSTRAINT user_favourites_ibfk_1 FOREIGN KEY (centre_id) REFERENCES centres (centre_id) ON DELETE CASCADE
...
CREATE TABLE walkin_named_buyers (
   ...
   is_active tinyint(1) DEFAULT '1',
   created_at datetime DEFAULT CURRENT_TIMESTAMP,
   code varchar(20) DEFAULT NULL COMMENT 'Unique customer code (per centre)',
   default_milk_type enum('cow','buffalo','mixed') DEFAULT 'mixed',
   address_line2 varchar(255) DEFAULT NULL,
   pincode varchar(10) DEFAULT NULL,
   PRIMARY KEY (buyer_id),
   UNIQUE KEY idx_centre_code (centre_id,code),
   KEY operator_id (operator_id),
   KEY centre_id (centre_id),
