...
// ══════════════════════════════════════════════════════════════
// GET /api/cattle-feed-sales/fulfillment/:token
    ...
    try {
        ...
        const [rows] = await pool.query(
            `SELECT f.fulfillment_id, f.transaction_id, f.status, f.fulfilled_at,
             ...
             FROM cattle_feed_fulfillments f
        ...
        const [items] = await pool.query(
            `SELECT cfs.sale_id, cfs.feed_id, cf.feed_name, cf.unit,
                    cfs.quantity, cfs.rate, cfs.total_amount,
                    cfs.buyer_type, cfs.buyer_name,
                    s.name AS seller_name, s.seller_code,
                    nb.name AS registered_buyer_name
             FROM cattle_feed_sales cfs
             JOIN cattle_feeds cf ON cf.feed_id = cfs.feed_id
...
// ══════════════════════════════════════════════════════════════
// POST /api/cattle-feed-sales/fulfillment/:token/confirm
// ══════════════════════════════════════════════════════════════
exports.confirmFulfillment = async (req, res) => {
    ...
    try {
        ...
        const [rows] = await conn.query(
            `SELECT * FROM cattle_feed_fulfillments WHERE token = ? AND centre_id = ? FOR UPDATE`,
            [token, centreId]
        );
        if (!rows.length) {
            await conn.rollback();
        ...
        if (fulfillment.status === 'fulfilled') {
            ...
            return res.status(409).json({
                error: 'This receipt has already been used to collect feed.',
                fulfilled_at: fulfillment.fulfilled_at,
            });
        }
        ...
        await conn.query(
            `UPDATE cattle_feed_fulfillments
        ...
        const [items] = await pool.query(
            `SELECT cf.feed_name AS feed_name, cf.unit, cfs.quantity
             FROM cattle_feed_sales cfs
             JOIN cattle_feeds cf ON cf.feed_id = cfs.feed_id
             WHERE cfs.transaction_id = ?`,
            [fulfillment.transaction_id]
        ...
        res.json({
            message: 'Feed collection confirmed.',
            transaction_id: fulfillment.transaction_id,
            items,
        });
    } catch (err) {
